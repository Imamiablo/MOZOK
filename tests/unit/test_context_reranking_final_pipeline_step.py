...
def test_final_pipeline_step_exposes_memory_reranking_report():
    ...
    final_step = debug["pipeline_steps"][-1]

    assert final_step["step"] == "final_prompt"
    assert final_step["memory_reranking"][0]["memory_id"] == 42
    assert final_step["memory_reranking"][0]["reason"].startswith("Selected because")
